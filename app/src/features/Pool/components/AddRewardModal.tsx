...
import useAddRewardAmount from "../hooks/useAddRewardAmount";
...
const AddRewardModal = ({ show, setShow, poolId, callback }: Props) => {
  const { call: addReward, isLoading: isDepositting } = useAddRewardAmount();
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    await addReward(utils.parseEther(values.amount), poolId);
    setShow(false);
    callback?.();
  });
